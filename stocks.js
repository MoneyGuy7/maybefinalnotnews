...
function getApiKey() {
  return (localStorage.getItem("yakton_api_key") || TWELVE_DATA_API_KEY || "").trim();
}
...
async function liveQuote(symbol) {
  ...
  const url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbol)}&interval=${interval}&outputsize=${outputsize}&apikey=${getApiKey()}`;
...
function initStocks() {
  ...
  els.apiKeyRow = document.getElementById("apiKeyRow");
  els.apiKeyInput = document.getElementById("apiKeyInput");
  els.apiKeySave = document.getElementById("apiKeySave");
...
function initApiKeyBox() {
  const saved = localStorage.getItem("yakton_api_key") || "";
  els.apiKeyInput.value = saved;
  els.apiKeyRow.classList.toggle("is-set", !!saved);
  ...
  function saveApiKey() {
    ...
    els.apiKeyRow.classList.toggle("is-set", !!key);
    els.apiKeySave.textContent = "Saved";
    setTimeout(() => (els.apiKeySave.textContent = "Save key"), 1200);
    loadSymbol(state.symbol);
  }
}
...
window.YaktonStocks = { pseudoDailyChange: pseudoDailyChangeCache, getQuote, KNOWN_NAMES };
window.dispatchEvent(new Event("yakton-stocks-ready"));

// ---------- crypto strip (order book / spread mock, shown for *-USD symbols) ----------
function isCryptoSymbol(sym) { return /-USD$/i.test(sym); }

function renderCryptoStrip(q) {
  const strip = document.getElementById("cryptoStrip");
  if (!strip) return;
  if (!isCryptoSymbol(q.symbol)) { strip.style.display = "none"; return; }
  strip.style.display = "flex";
  document.getElementById("cHighLow").textContent = `$${fmt(q.low)} – $${fmt(q.high)}`;
  const spread = q.price * 0.0012; // mocked tight spread, real books aren't exposed by our free sources
  const bid = q.price - spread / 2, ask = q.price + spread / 2;
  document.getElementById("cSpread").textContent = `$${fmt(bid)} / $${fmt(ask)}`;
  const bidsEl = document.getElementById("cBids"), asksEl = document.getElementById("cAsks");
  bidsEl.innerHTML = ""; asksEl.innerHTML = "";
  for (let i = 0; i < 5; i++) {
    const bp = bid - (bid * 0.0006 * i), ap = ask + (ask * 0.0006 * i);
    const size = (Math.random() * 2 + 0.05).toFixed(3);
    bidsEl.innerHTML += `<div class="orderbook-row"><span>$${fmt(bp)}</span><span>${size}</span></div>`;
    asksEl.innerHTML += `<div class="orderbook-row"><span>$${fmt(ap)}</span><span>${size}</span></div>`;
  }
}

const _origRenderQuote = renderQuote;
renderQuote = function (q) { _origRenderQuote(q); renderCryptoStrip(q); };
